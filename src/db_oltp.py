...
class DatabaseInitializer:
    def __init__(self, path_db="..\\data\\IATA.db"):
    ...
    def init_db(self):
        ...
        if not self.path_db.exists():
            print("🛠 Creando base de datos y tablas iniciales...")
            self.conn = DataLoader.init_db(path_db=str(self.path_db))

            # Crear tablas
            for name, create_sql in self.tables:
                DataLoader.create_table(conn=self.conn, create_table_sql=create_sql, table_name=name)

            # Insertar datos iniciales
            self._insert_initial_data()
            print("✅ Base de datos creada correctamente.")
        else:
            print("📁 Base de datos ya existe. No se realizaron cambios.")

    def _insert_initial_data(self):
        """Ejecuta los inserts iniciales definidos"""
        cursor = self.conn.cursor()
        for insert_sql in self.inserts:
